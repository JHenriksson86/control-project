...
class PathPlanner {
  ...
  ros::Publisher _goal_pub, _next_pub, _traj_pub;
  ...
  void publishTrajectory(){
    ...
    geometry_msgs::Pose pose, next_pose;
    std::vector<cv::Point>::iterator iter = this->_waypoints.begin();
    pose = convertPixelToMetric(*iter);
    iter++;

    for( ; iter != this->_waypoints.end(); iter++) {
      next_pose = convertPixelToMetric(*iter);
      pose.orientation = getOrientation(pose.position, next_pose.position);
      trajectory.poses.push_back(pose);
      pose = next_pose;
    } 

    ROS_INFO("Publishing trajectory");
    this->_traj_pub.publish(trajectory);
  }
...
public:
  ...
  PathPlanner(cv::Mat &map) : _img_map(map), _searching(false), _priv_nh("~") {
    ...
    this->_traj_pub = this->_nh.advertise<geometry_msgs::PoseArray>( "/planner/trajectory", 10);
