...
class PathPlanner {
  ...
  ros::Publisher _goal_pub, _next_pub, _path_pub;
  ...
  void Trajectory() {
    ...
    double last_heading =  atan2(this->_traj[1].x - this->_traj[0].x, this->_traj[1].y - this->_traj[0].y);
    
    for(int i = 0; i < this->_traj.size() - 20; i++){
      double horizon = i + 20;

      double heading = atan2(this->_traj[horizon].x - this->_traj[i].x, this->_traj[horizon].y - this->_traj[i].y);
      if(std::abs(heading - last_heading) > 0.4 && (i % 20)){
        ROS_INFO("Last heading: %.2f, Heading: %.2f", last_heading, heading);
        last_heading = heading;
        this->_waypoints.push_back(cv::Point(this->_traj[i].y, this->_traj[i].x));
      }
    }
  ...
  void publishPath(){
    nav_msgs::Path path;
    geometry_msgs::PoseStamped pose, next_pose;
    pose.pose = convertPixelToMetric(this->_robot);

    int id = 0;
    std::vector<cv::Point>::iterator iter = this->_waypoints.begin();
    for( ; iter != this->_waypoints.end(); iter++) {
      next_pose.pose = convertPixelToMetric(*iter);
      pose.pose.orientation = getOrientation(pose.pose.position, next_pose.pose.position);
      pose.header = getHeader(id, "map");
      path.poses.push_back(pose);
      id++;
      if(iter != this->_waypoints.end())
        pose = next_pose;
    } 
    next_pose.pose.orientation = pose.pose.orientation;
    next_pose.header = getHeader(id, "map");
    path.poses.push_back(next_pose);
    path.header = getHeader(0, "map");

    ROS_INFO("Publishing path");
    this->_path_pub.publish(path);
  }

  geometry_msgs::Quaternion getOrientation(geometry_msgs::Point pos1, geometry_msgs::Point pos2){
      double X = pos2.x - pos1.x;
      double Y = pos2.y - pos1.y;
      double angle = std::atan2(Y, X);
      geometry_msgs::Quaternion orientation;
      orientation.z = std::sin(angle/2.0);
      orientation.w = std::cos(angle/2.0);
      return orientation;
  }

  std_msgs::Header getHeader(int seq_id, const std::string& frame_id){
...
public:
  ...
  PathPlanner(cv::Mat &map) : _img_map(map), _searching(false), _priv_nh("~") {
    ...
    this->_path_pub = this->_nh.advertise<nav_msgs::Path>( "/planner/path", 10);
